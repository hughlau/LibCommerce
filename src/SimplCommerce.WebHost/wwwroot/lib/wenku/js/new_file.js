...
$(function() {
	...
	if($("body").attr("class") == "findpwdreg") {
		...
		$(".btn-sendmail").click(function() {
			var _this = $(this),
				...
				email = form.find("input[name=mailadddress]"),
				username = form.find("input[name=usercode]");
            ...
            if(res){
               ...
               wenku_alert("danger", "图形验证码错误", 3000, "");
               return false;
            }
			if(email.val() == "") {
				wenku_alert("danger", "请输入注册邮箱或手机号码", 3000, "");
				email.focus();
				return false;
			} else {

				ajaxs(inter_url + 'Interface/postMail.ashx', {
					...
					if(rt.code == 0) {
						...
						code1 = rt.desc;
						wenku_alert("success", '验证码发送成功', 3000, ""); //+ rt.desc
		...
		$(".btn-reg").click(function() {
			...
			var _this = $(this),
				...
				mailadddress = form.find("input[name=mailadddress]"),
				code = form.find("input[name=code]"),
				userpwd = form.find("input[name=userpwd]"),
				reuserpwd = form.find("input[name=reuserpwd]"),
				nikname = form.find("input[name=nikname]");
			if(mailadddress.val() == "") {
				wenku_alert("danger", "邮箱或手机号码不能为空", 1000, "");
				return false;
			}
			if(code1 != code.val()) {
				wenku_alert("danger", "验证码不正确,请重新输入", 1000, "");
				return false;
			}
			if(userpwd.val() != reuserpwd.val()) {
				wenku_alert("danger", "密码与重复密码不一致,请重新输入", 1000, "");
				return false;
			}
			...
			ajaxs(inter_url + 'interface/register.ashx', {
				"usercode": usercode.val(),
				"userpwd": userpwd.val(),
				"nikname": nikname.val(),
				"mailaddress": mailadddress.val(),
				"f_id1": app.f_id1,
				f_id2: app.f_id2,
				f_id3: app.f_id3
			}, function(rt) {
	...
	if($("body").attr("class") == "findpwd") {
			...
			if(email.val() == "") {
				...
				ajaxs(inter_url + 'Interface/postMail.ashx', {
					...
					if(rt.code == 0) {
						_this.addClass("disabled");
						code1 = rt.desc;
						wenku_alert("success", '验证码发送成功', 3000, "");
						var ori_text = _this.text();
						var i = 60
						var interval = setInterval(function() {
							_this.text(ori_text + "(" + i + ")")
		...
		$(".btn-reg").click(function() {
			var _this = $(this),
				...
				inputs = form.find("input[required=required]"),
				usercode = form.find("input[name=usercode]"),
				mailaddress = form.find("input[name=mailadddress]"),
				code = form.find("input[name=code]"),
				userpwd = form.find("input[name=userpwd]"),
				reuserpwd = form.find("input[name=reuserpwd]"),
				nikname = form.find("input[name=nikname]");
			...
			ajaxs(inter_url + 'interface/findPwd.ashx', {
				"usercode": usercode.val(),
				"userpwd": userpwd.val(),
	...
	if(PageId == "wenku-user") {
		...
		$(".buttonxiugai").click(function() {
			var _this = $(this),
				...
				userpwd = form.find("input[name=userpwd]"),
				userNewpwd = form.find("input[name=userNewpwd]"),
				reuserNewpwd = form.find("input[name=reuserNewpwd]")

			;
			if(userNewpwd.val() != reuserNewpwd.val()) {
				wenku_alert("danger", "密码与重复密码不一致,请重新输入", 1000, "");
				return false;
			}
			$.each(inputs, function() {
			...
			ajaxs(inter_url + 'interface/modifyPwd.ashx', {
				"usercode": usercode.val(),
				"userpwd": userpwd.val(),
	...
	//			var _this = $(this),
	//				form = _this.parents("form"),
	//				url_checklogin = form.attr("data-login"),
	//				requireds = form.find("[required=required]"),
	//				req_len = requireds.length;
	//			_this.addClass("disabled");
	//			if(req_len > 0) {
	...
	//						$.get(form.attr("data-doccheck"), {
	//							md5: form.find("[name=Md5]").val()
	//						}, function(ret) {
	//							//文档已在文档库中存在
	//							if(ret.status == 1) {
	...
	if(PageId == "wenku-content") {
		...
		$('#ModalDownload').on('show.bs.modal', function() {
			// 请求链接其实不应该写死在这里的...
			$.get("/downfree", {
				id: $("#ModalDownload .btn-submit-download").attr("data-id")
			}, function(ret) {
				if(ret.status == 1) {
		...
		$(".wenku-page-prev").click(function() {
			...
			if(prevPage > 0) {
				var hWindow = $(window).height(),
					hPage = $(".wenku-page" + prevPage).height();
				if(hPage > hWindow) { //文档页面高度与窗口高度做比较
					ScrollToPage(prevPage);
				} else {
					//这个不能调用ScrollToPage
					$('html,body').animate({
						scrollTop: $(".wenku-page" + prevPage).offset().top + hPage - hWindow
					}, 200);
				}
			}
		});
		...
		$(".wenku-page-next").click(function() {
			...
			if(nextPage < limit) {
				...
				if(hPage > hWindow) { //文档页面高度与窗口高度做比较
					...
					//这个不能调用ScrollToPage
					$('html,body').animate({
						scrollTop: $(".wenku-page" + nextPage).offset().top + hPage - hWindow
					}, 200);
				}
			} else {
		...
		$(".wenku-viewer-more-btn").click(function() {
			...
			if($(window).scrollTop() > $(".wenku-viewer-more").offset().top) {
				$('html,body').animate({
					scrollTop: $(".wenku-viewer-more").offset().top - $(window).height() / 2
				}, 500);
				return
			}
		...
		$(".wenku-goto-comment").click(function() {
			$('html,body').animate({
				scrollTop: $(".wenku-comment").offset().top
			}, 200);
		});
		...
		$("form.wenku-comment-form [type=submit]").click(function(e) {
			e.preventDefault();
			var form = $("form.wenku-comment-form"),
				score = $("#score").val(),
		...
		$(".btn-submit-collect").click(function() {
			var did = $("#ModalCollect [name=Did]").val(),
				cid = $("#ModalCollect [name=Cid] option:selected").val();
			if(cid == "") {
	...
	$("#notarget").load(function() {
		...
		if(obj != undefined) {
			if(obj.code == 0) {
				setCookie("avatar", obj.f_avatar);
				wenku_alert("success", obj.desc, 5000, "");
				setTimeout(function() {
					window.location.reload();
				}, 3000)
			} else {
				wenku_alert("danger", obj.desc, 2500, "");
			}
		}
	});
	...
	function WenkuLazyLoad() {
		var imgs = $("#wenku-viewer .wenku-lazy"),
			hScrollTop = $(window).scrollTop(),
			hWinow = $(window).height();
		$.each(imgs, function() {
	...
	function ScrollToPage(page) {
		$('html,body').animate({
			scrollTop: $(".wenku-page" + page).offset().top
		}, 200);
	}
